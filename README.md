...
**클로드 코워크(Cowork)**에서는 설정 → 플러그인 → 마켓플레이스 추가에
`stiron92-byte/claude-skills`를 입력하고 `claude-skills-book`을 설치하세요.
...
이 영상을 카드뉴스로 정리해줘 → generate-shorts (카드뉴스 모드)
```

업데이트가 있으면 `/plugin marketplace update claude-skills` 후 플러그인을 업데이트하고
세션을 재시작하면 됩니다. (플러그인 버전이 올라갔는지 확인하세요)
...
| `generate-shorts` | 유튜브 롱폼 → 세로 쇼츠 자동 생성 (Claude가 구간 선별·제목·자막 작성) + **카드뉴스 모드**(요약 카드 이미지 + AI 나레이션 영상) | ffmpeg · yt-dlp — `scripts/setup.sh` 참고, 나레이션은 `pip install edge-tts`(무료) |
